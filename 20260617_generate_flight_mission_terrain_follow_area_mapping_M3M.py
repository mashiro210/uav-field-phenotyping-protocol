# Author: Mashiro
# Last update: 2026/6/17 (高度モード不具合修正版)
# Description: generate area mission terrain follow with explicit EGM96/WGS84 mode; M3M
...
import numpy as np
import gpd as gpd # もし元が geopandas であれば geopandas に適宜読み替えてください
...
def generate_route(polygon_wgs84, course_dist, photo_dist):
    ...
    return route_3d, exec_height_mode, coord_height_mode
...
def export_kmz_with_et(waypoints, exec_height_mode, coord_height_mode, speed, interval_sec):
    ...
    add_elem(sys_param_t, "wpml:heightMode", coord_height_mode) # 明示的に EGM96 が入る
    ...
    add_elem(folder_w, "wpml:executeHeightMode", exec_height_mode) # 明示的に WGS84 が入る
...
if calc_speed > max_safe_speed:
    ...
    print(f"⚠️ 警告: 設定速度ではカメラの保存が追いつきません。減速します。")
...
final_waypoints, exec_h_mode, coord_h_mode = generate_route(target_polygon, course_spacing_m, photo_spacing_m)

print(f"\n[エクスポート] KMZファイルの構築を開始します...")
export_kmz_with_et(
    waypoints=final_waypoints,
    exec_height_mode=exec_h_mode,
    coord_height_mode=coord_h_mode,
    speed=final_speed,
